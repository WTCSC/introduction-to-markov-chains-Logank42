...
text = """Once upon a time there was a lovely princess. But she had an enchantment upon her of a fearful
...
Fairytale creatures. Right, this one is full. Take it away. Give me that. Your fine days are over. twentyfive
...
that ugly. Wait, wait, I'll not lie, you are ugly. But you only look like this at night. Shrek's ugly twentyfour seven.
...
words = re.split(r"[.,:;?!\-\n\"\... ]", text.lower()) # Removes all special characters replaces with spaces, lowercases all words
words = [x for x in words if x != ''] # Adds words that aren't spaces back to the words, and removes all spaces
for i in range(len(words)):
    # Capatlizes all proper nouns
    if words[i] == "shrek":
        words[i] = "Shrek"
    if words[i] == "fiona":
        words[i] = "Fiona"
...
def generate_text(start_word, num_words):
    ...
    for _ in range(int(num_words) - 1):
        if current_word in transitions:
            next_word = random.choice(transitions[current_word]) # Chooses random word baised on previous word
            result.append(next_word) # Adds to generated sentence
            current_word = next_word
        else:
            break
    return " ".join(result)

# Takes input of the first word and how many words
starting_word = input("Enter the first word of your sentence: ")
number_word = input("Enter a number of words:")
start_word = starting_word
num_words = number_word 

# Prints sentence with first word capitalized and ending with a period
result = generate_text(start_word, num_words)
